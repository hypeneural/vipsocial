...
## 2.3. Wizard de Cadastro (Painel SPA / React)

A arquitetura do painel de administração é invertida em relação a antigos sistemas (`n8n` manual). A UX é guiada pelo AutoDiscovery, escondendo complexidade.

1.  **URL Base:** O operador fornece "prefeitura.sc.gov.br", e o backend reage em `background`.
2.  **AutoDiscovery (Bot em Ação):** Usando `Spatie Crawler`/`DomCrawler`, o Laravel viaja pela home. Tenta detectar `<link rel="alternate" type="application/rss+xml">`. Tenta requisitar `/sitemap.xml` / `/news-sitemap.xml`. Tenta identificar `href` que pareçam apontar para links únicos baseados nos formatos de URLs.
3.  **Apresentação do Preview:** A tela retorna três (3) _Cards_ de _Preview_. O formulário preenche automaticamente os campos que encontrou "magicamente" pelas marcações padronizadas (JSON-LD, Meta Og). O sistema indica "Dados Obtidos via Open Graph", "Dados via XML Feed", etc.
4.  **Ajuste Fino Manual:** O Operador só interfere se o portal não obedecer padrões universais. Seletores de CSS fallback são adicionados visualmente (extratores Custom). Exceções de URL `/videos/`, etc.
